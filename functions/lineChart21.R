lineChart21 <- function(data, x,
  ...
  if (!is.na(y2)) {
    ...
    #   
    #   data <- data %>%
    #     mutate(dash1 = case_when(year %in% c(2019, 2020) ~ young,
    ...
    #            dash3 = case_when(year < 2020 ~ as.numeric(NA),
  ...
  line <- line %>%
    ...
    layout(legend = list(orientation = "h",    # Here the legend is changed to horizontal 
           ...
           yaxis = list(title = "",               # Remove title from y axis and extend range from 0 to 100
                        ...
                        tickformat = ",d"),
           annotations = list(
           list(text = "No YLT\ndata for 2020", showarrow = FALSE,
                x = 2020, xref = "x", xanchor = "center",
                y = 95, yref = "y", yanchor = "middle")),
           shapes = list(
             list(type = line,
                  x0 = 2020, x1 = 2020, y0 = 0, y1 = 88,
